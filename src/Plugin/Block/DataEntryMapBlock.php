...
class DataEntryMapBlock extends IndiciaControlBlockBase {
  ...
  protected function getControlConfigFields() {
    ...
    return [
      ...
      'initialLat' => [
        ...
        '#default_value' => is_numeric($config->get('map_centroid_lat')) ? round($config->get('map_centroid_lat'), 5) : 53.9538,
      ],
      'initialLon' => [
        ...
        '#default_value' => is_numeric($config->get('map_centroid_long')) ? round($config->get('map_centroid_long'), 5) : -2.4571,
      ],
      'initialZoom' => [
        '#title' => 'Initial zoom',
        '#type' => 'number',
        ...
        '#default_value' => is_numeric($config->get('map_zoom')) ? $config->get('map_zoom') : 10,
      ],
      'customLayers' => [
  ...
  public function build() {
    ...
    $ctrlOptions = [
      ...
      'editLayer' => TRUE,
      'layers' => [],
      'initial_lat' => is_numeric($config->get('map_centroid_lat')) ? $config->get('map_centroid_lat') : 55.5,
      'initial_long' => is_numeric($config->get('map_centroid_long')) ? $config->get('map_centroid_long') : -2.5,
      'initial_zoom' => is_numeric($config->get('map_zoom')) ? $config->get('map_zoom') : 5,
      'width' => '100%',
      'height' => '500',
